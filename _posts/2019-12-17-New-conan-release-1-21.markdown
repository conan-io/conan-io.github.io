...
in Windows and `gcc` in Linux as base compilers. To manage the compatibility between the
...
feature](https://docs.conan.io/en/latest/reference/conanfile/attributes.html?highlight=cpp_info%20names#cpp-info)
by using `cpp_info.names["generator_name"]` so you can specify this same name per generator.
If you want to use `CMake` and `pkg-config` generators for the same recipe different names can be set
for each of those.
...
will be created. If you install the package using the `pkg-config` generator a file `my_lib.pc` will
be generated with a library name `my_lib` inside.

## Other cool things

 * Set the [logging
   level](https://docs.conan.io/en/latest/reference/env_vars.html?highlight=warning#conan-logging-level)
 ...
 * Use
   [`tools.check_min_cppstd`](https://docs.conan.io/en/latest/reference/tools.html?highlight=check_min_cppstd#tools-check-min-cppstd)
   and
   [`tools.valid_min_cppstd`](https://docs.conan.io/en/latest/reference/tools.html?highlight=check_min_cppstd#tools-valid-min-cppstd)
   to check if the cppstd version is valid for a specific package.
 * Use [`fuzz`](https://docs.conan.io/en/latest/reference/tools.html?highlight=fuzz#tools-patch)
