...
layout: post
comments: false
title: "OpenCV conan packages: options, dependencies and challenges"
description: "OpenCV conan packages are available in conan-center, but there are still things to be done"
---

### What is OpenCV?

If you're not familar with OpenCV yet, check out our previous blog-post about [OpenCV 4.0.0](https://blog.conan.io/2018/12/19/New-OpenCV-release-4-0.html).
Given the fact OpenCV is huge library with lots of features for various use-case, it's good example to demonstrate some typical package challenges (and probably few more specific as well).
...
Installation with conan should be pretty straighforward, e.g. you may use the following [conanfile.txt](https://docs.conan.io/en/latest/reference/conanfile_txt.html) to consume OpenCV 4.0.0:
...
OpenCV is very complex library, and has lots of various dependencies. Current conan recipe has the following:
...
Graph was generated by the [conan info](https://docs.conan.io/en/latest/reference/commands/consumer/info.html) command:
...
As stated previously, OpenCV is very large and complex library, and it really has tons of options. And currently, our conan package doesn't support them all. You may check list of available options on their [GitHub repository](https://github.com/opencv/opencv/blob/4.0.1/CMakeLists.txt#L208). It literally takes almost 300 lines of CMake code just to declare all these options! This is something that actually hard to model in one shot. Moreover, most of options depend on other 3rd-party libraries.
...
* [OpenCL](https://www.khronos.org/opencl/) and [CUDA](https://developer.nvidia.com/cuda-zone) might be used to accelerate OpenCV algorithms on [heterogenous systems](https://en.wikipedia.org/wiki/Heterogeneous_System_Architecture)
...
We're currently actively working on adding Google Protobuf [recipe](https://github.com/bincrafters/conan-protobuf) accepted into [conan-center](https://bintray.com/conan-community/conan/opencv%3Aconan). Library is itself challenging, especially for cross-compilation use case. As soon as it's accepted, we are going to enable our OpenCV package to use Protocol Buffers by default.
...
Similar story to OpenCL, however, there is only one vendor, obviously - building of [CUDA](https://developer.nvidia.com/cuda-zone) applications requires [nVidia CUDA Toolkit](https://developer.nvidia.com/cuda-toolkit). Toolkit is pretty large, and contains CUDA compiler, in addition to libraries and headers. We either have to require user to have CUDA installed on machine during build, or provide a package for toolkit.
