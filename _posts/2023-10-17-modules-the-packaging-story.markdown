...
For the longest time, textual inclusion of header files has been the way to share declarations across multiple translation units. Thus, the concept of a library that provides functionality has been tied to “include files”, as well as the binary library files themselves. This is how most libraries are packaged for development. Example:
...
C++ 20 introduces Modules as a new way to share declarations and definitions across multiple translation units. With modules, we could have something like this on the caller’s side:
...
Unlike the `#include` preprocessor directive, which simply performs textual inclusion of the referenced file, the `import` keyword is a C++ language feature which behaves differently. Modules have advantages over legacy header inclusion, by providing better isolation: 
...
Let’s assume we have a library that already supports C++ modules - what are the packaging considerations? In order to answer the question, we should first have a look at how the compiler resolves an import in the first place:
...
In order to resolve `import fmt`, the compiler needs to locate the binary module interface (BMI) for the module named `fmt`. For clang, we need to tell the compiler where this file is for this named module, with the `-fmodule-file=fmt=/path/to/fmt.pcm` flag. Note that there are other flags and mechanisms for the compiler to locate modules, and these vary per compiler.

Binary modules interfaces are files generated by the compiler when translating module interface units - that is, those whose declaration have the export keyword. For illustration purposes, an example of a module interface could be:
...
* BMIs are not compatible across compilers (they are implementation specific) and are also not guaranteed to be compatible across different versions of the same compilers. Inconsistent compiler flags can also render a BMI invalid for the importer - that is, if the importer is being compiled with different (potentially incompatible) flags than when the BMI was first generated.

Solving the first problem (correct compilation order), has required giving the compilers the ability to perform dependency scanning on source files and expressing them in a file format (see [p1689r5](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2022/p1689r5.html)). This information can then be used by the build system to derive the correct build order. CMake started experimenting with this [earlier this year](https://www.kitware.com/import-cmake-c20-modules/) with version 3.25, and the feature will be available in the [upcoming 3.28 release](https://www.kitware.com/cmake-3-28-0-rc1-is-ready-for-testing/).
...
For compilers like Clang and msvc that allow passing flags to specify the location of the BMIs for specific named modules, we can rely on existing abstractions to propagate this information to the consumer, for example - in the `package_info()` method of a Conan recipe:
...
While the package contents would look roughly like this (for Clang on Linux):
```
|-- bmi
|   `-- fmt.pcm
`-- lib
...
For this to work - we would need to use Conan in a way that enforces strict compatibility. A package like the above would ONLY be compatible with that specific compiler and version. In order to do this, we would have to:
...
* Disable the defaults values shipped with Conan 2.0 for the `compatibility` plugin, which handles the logic for considering other binary packages as compatible candidates ([docs](https://docs.conan.io/2/reference/extensions/binary_compatibility.html) here). 
...
* Clang will reject BMIs where the original source file does not exist in the local filesystem - this would make it impossible to build packages on one machine (e.g. CI) and consume it on another. While the importer only needs a BMI, the original sources are still used by the compiler when reporting compiler errors.
* GCC does not currently support compiler flags to specify where the BMIs are for specific modules, but rather, it supports a global module mapper. While Conan could create this, it would still need to cooperate with the build system on the consumer side. 

On the other hand, msvc appeard to be more forgiving and reusing the repackaged BMI appared to work just fine in the scenarios we tested. 

While this approach may be useful for teams that have total control of their dependencies, the exact compiler and compiler version used (in all environments!) - packaging BMIs is not something that we would recommend in order to use modules.
...
So far, CMake 3.28 (still a Release Candidate at time of writing) is the only build system that implements the ability to consume external libraries that provide modules, in a way that the module interfaces take part in dependency scanning, and the BMIs are built locally rather than distributed. A fairly modern setup is required as well! The compilers must support p1689r5 (Clang >= 16.0, msvc from the 14.34 toolset, and the yet-to-be-released gcc 14), as well as the build tool (Ninja 1.11.1 or MSBuild). All very bleeding edge!
