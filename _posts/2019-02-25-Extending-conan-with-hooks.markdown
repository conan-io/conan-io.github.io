...
The Conan hooks is a python script intended to extend the Conan functionalities and let users customize the client behavior at determined points. To use them we would need to have the source code of the hook to be executed by Conan and get them activated with some commands.
