...
Please, note that the values above are set for a single-configuration CMake generator. To support
...
helpers. The filename was the same for all the toolchains so that caused a collision problem when a
recipe used different build systems. Conan 1.41 provides a new argument _namespace_ to the tools
helpers for [CMake](https://docs.conan.io/en/latest/reference/conanfile/tools/cmake/cmake.html),
...
of the same package are built with different build systems. Setting this value, the namespace will be
appended to the filename (like __conanbuild_<namespace>.conf__) generating an unique name for each
